...
namespace
{
  ...
  glm::vec3 ambientColor = glw::OFF_BLACK;
...
GReturnCode loop()
{
  ...
  engine.setClearColor(glw::GREY);
  ambientColor = glw::GREY;
  ...
  // Draw multiple instances of the same FBO
  for (float ix = 0.5f; ix > -0.5f; ix-=0.1f)
  {
    // Increase the size
    fbo.setSize(windowSize * (ix + 0.5f));
    // Move diagonally
    fbo.setTopLeft(windowSize * ix);
    // Rotate
    fbo.rotation(vbos[0].m_theta);
    // Draw
    shaderProgramManager.drawFBO(fbo);
  }


  // Draw original VBOs over the top //

  ambientColor = glw::SKY_BLUE;
