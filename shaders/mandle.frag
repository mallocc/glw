...
float get_complex_point(float component, float frame_start, float axis_length, float target_component, float frame_length)
{
...
void main() 
{
	...
	vec2 c = vec2(get_complex_point(o_pos.z, -1.0f, 2.0f, u_target.x, window.x), get_complex_point(-o_pos.x, -1.0f, 2.0f, u_target.y, window.y));
